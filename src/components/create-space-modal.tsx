...
import { createWorkspaceFromTemplate } from "@/lib/storage";
...
export function CreateSpaceModal({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const create = async () => {
    try {
      const space = await createWorkspaceFromTemplate(tplKey, name.trim() ? { name: name.trim() } : undefined);
      toast({ title: "Space created", description: space.name, variant: "success" });
      onClose();
      setName(""); setTplKey("blank");
      // The original used /space/$spaceId, we'll keep that but map it to our workspaceId
      navigate({ to: "/space/$spaceId", params: { spaceId: space.id } as any, search: true as any });
    } catch (e) {
