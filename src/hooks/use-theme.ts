...
export function useTheme() {
  const [family, setFamily] = useState<ThemeFamily>(() => readLocal(FAMILY_KEY, "vaultAtelier") as ThemeFamily);
  ...
  useEffect(() => {
    if (!db.isOpen()) return;
    updateSettings({ themeFamily: family, themeMode: mode, serifHeadings: serif, compactMode: compact, theme: family as any }).catch(() => {});
