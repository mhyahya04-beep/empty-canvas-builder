...
export function StorageManagerModal({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const blobs = useLiveQuery(async () => open ? await db.attachmentBlobs.toArray() : [], [open]) ?? [];
  const attachments = useLiveQuery(async () => open ? await db.attachments.toArray() : [], [open]) ?? [];

  const refByHash = new Map<string, number>();
  for (const a of attachments) {
    if (!a.hash) continue;
    refByHash.set(a.hash, (refByHash.get(a.hash) ?? 0) + 1);
  }
      ...
      <div className="space-y-5">
        ...
        <div className="rounded-xl border border-border overflow-hidden">
          ...
          <div className="max-h-72 overflow-y-auto divide-y divide-border">
            ...
            {blobs.map((b) => {
              const refs = refByHash.get(b.hash) ?? 0;
              const orphan = refs === 0;
              const sample = attachments.find((a) => a.hash === b.hash);
              return (
                <div key={b.hash} className="flex items-center gap-3 px-4 py-2.5">
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="text-sm truncate">{sample?.name ?? "Untitled file"}</div>
                    <div className="text-[11px] text-muted-foreground">
                      {fmtBytes(b.size || b.dataUrl.length)} · {b.mimeType || "unknown"} ·{" "}
                      {orphan ? <span className="text-destructive">unreferenced</span> : `used by ${refs} record${refs === 1 ? "" : "s"}`}
                    </div>
                  </div>
                </div>
              );
